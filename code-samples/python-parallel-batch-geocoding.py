...
# Installation:
# pip3 install opencage asyncio aiohttp geopy backoff
...
async def write_one_geocoding_result(geocoding_result, address, address_id):
  if geocoding_result != None:
    row = [
      ...
      geocoding_result.raw['components'].get('country', ''),
      geocoding_result.raw['components'].get('county', ''),
      geocoding_result.raw['components'].get('city', ''),
      geocoding_result.raw['components'].get('postcode', ''),
      geocoding_result.raw['components'].get('road', ''),
      geocoding_result.raw['components'].get('house_number', ''),
      #
      geocoding_result.raw['confidence'],
      geocoding_result.raw['formatted']
    ]
  else:
    row = [
      address_id,
...
@backoff.on_exception(backoff.expo,
                      (GeocoderTimedOut, asyncio.TimeoutError),
...
async def geocode_one_address(address, address_id):
  ...
  async with OpenCage(
    ...
    # https://geopy.readthedocs.io/en/stable/#opencage
    geocoding_result = await geopy_geocoder.geocode(address, language="en", country="us")
